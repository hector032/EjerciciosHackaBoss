...
public class VotoJpaController implements Serializable {
    ...
    public Voto findVoto(Long id) {
        ...
        //Creamos una query personalizada para poder filtrar por partido (A, B, C)
        public Voto findVotoPorPartido(String partido) {
        EntityManager em = getEntityManager();
        try {
            Query query = em.createQuery("SELECT v FROM Voto v WHERE v.partido = :partido");
